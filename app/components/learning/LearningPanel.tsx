...
export const LearningPanel: React.FC<LearningPanelProps> = ({ onContentReceived }) => {
  const session = useSessionContext();
  const [flashCards, setFlashCards] = useState<FlashCard[]>([]);
  const [currentQuiz, setCurrentQuiz] = useState<Quiz | null>(null);
  const handlersRegisteredRef = useRef(false);
  const onContentReceivedRef = useRef(onContentReceived);

  // Keep the callback ref updated
  useEffect(() => {
    onContentReceivedRef.current = onContentReceived;
  }, [onContentReceived]);

  useEffect(() => {
    if (!session.room || handlersRegisteredRef.current) return;
  ...
  return (
    <div className="learning-panel h-full flex flex-col p-6 pt-16 bg-gray-50 dark:bg-gray-900">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Learning Materials</h1>
      ...
      <div className="flex-1 overflow-y-auto space-y-6">
        ...
        {currentQuiz && (
          <div className="quiz-section bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4">
            <QuizComponent
              quiz={currentQuiz}
